...
def __d9_sync_and_wait(d9_api_key_Id, d9_api_secret, cloud_account_number, region, excluded_types,
    ...
    # Perform SyncNow
    __perfrom_sync_now(cloud_account_number=cloud_account_number, d9_api_keyId=d9_api_key_Id,
                       d9_api_secret=d9_api_secret)
...
def __query_fetch_status(cloud_account_id, relevant_dome9_types, d9_api_keyId, d9_api_secret, excluded_types,
                         region=None):
    """
        ...
        if region:
            ...
            relevant = list(filter(lambda entry: entry['region'] in [d9region, ''],
                                   filter(lambda entry: entry[
                                                            'entityType'] in relevant_dome9_types if relevant_dome9_types else
                                   entry['entityType'] not in excluded_types,
                                          resp)))
        else:
            relevant = list(filter(
                lambda entry: entry['entityType'] in relevant_dome9_types if relevant_dome9_types else entry[
                                                                                                           'entityType'] not in excluded_types,
                resp))
...
def __get_stack_types_from_aws(aws_acc_number, region, stack_name, aws_profile):
    ...
    if (aws_acc_number != account_id):
        logging.error(
            f'Error - the provided awsAccNumber ({aws_acc_number}) is not tied to the AWS credentials of this script ({account_id}) consider providing a different "profile" argument')
        exit(1)
    ...
    relevant_cfn_types = list(
        set([resource['ResourceType'] for response in response_pages for resource in
